	...
	override def receive: Receive = {
		...
		case Create_New_Movement_Record(newHourBlockId, newQuarterBlockId) =>
			...
			MarketOperations.readLastNDaysMovementRecords(3) match {
				...
				case movementRecords : Vector[MovementRecord] =>
					val marketUpdate = (currentQuarterBlockId != newQuarterBlockId) || (currentHourBlockId == -1 && currentQuarterBlockId == -1)
					if (marketUpdate){
						...
						todayMarket = if (dateMarketCreated != todayDateId()) {
							...
							val result = if(movementRecords(0).orderNum >= 2) {
						...
						val latestMovementRecord =
							//carry over remnants of the last record if the previous day into the next day
							...
							//otherwise just pass along the data for every 15 minutes
							}else{
								movementRecords(0)
							}
						...
						val newDay = (newHourBlockId == 0 && newQuarterBlockId == 0) && latestMovementRecord.id != todayDateId()
						if (newDay) {
